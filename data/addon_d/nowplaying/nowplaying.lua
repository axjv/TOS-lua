...
_G['ADDONS']['MIEI']["NOWPLAYING"] = _G['ADDONS']['MIEI']["NOWPLAYING"] or {};
...
function NOWPLAYING_ON_INIT(addon, frame)
	...
	g.settingsFileLoc = path.GetDataPath() .. "../addons/nowplaying/settings.json";
...
function ADDONS.MIEI.NOWPLAYING.processCommand(words)
	local g = _G["ADDONS"]['MIEI']["NOWPLAYING"];
	local acutil = require('acutil');
...
function ADDONS.MIEI.NOWPLAYING.save()
	local acutil = require('acutil');
	acutil.saveJSON(g.settingsFileLoc, g.settings);
end
