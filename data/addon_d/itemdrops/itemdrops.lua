...
_G['ADDONS']['MIEI']["ITEMDROPS"] = _G['ADDONS']['MIEI']["ITEMDROPS"] or {};
...
function ITEMDROPS_ON_INIT(addon, frame)
	...
	g.settingsFileLoc = path.GetDataPath() .. "../addons/itemdrops/settings.json";
				...
				if drawStuff == true then
					...
					if itemObj ~= nil then
						...
						if alwaysShow == true or g.showOrNot(g.settings.msgFilterGrade, itemGrade) == true then
							...
							CHAT_SYSTEM(string.format("%s dropped%s%s %s", ownerName, itemGradeMsg, groupNameMsg, g.linkitem(itemObj))); --works
						end
					end
				end
			end
...
function ADDONS.MIEI.ITEMDROPS.showOrNot(setting, itemGrade)
	local g = _G['ADDONS']['MIEI']["ITEMDROPS"];
	local acutil = require('acutil');
...
function ADDONS.MIEI.ITEMDROPS.drawItemFrame(handle, itemName)
...
function ADDONS.MIEI.ITEMDROPS.linkitem(itemObj)
	local imgheight = 30;
	local imgtag =  "";
...
function ADDONS.MIEI.ITEMDROPS.processCommand(words)
	local g = _G['ADDONS']['MIEI']["ITEMDROPS"];
	local acutil = require('acutil');
...
function ADDONS.MIEI.ITEMDROPS.checkFilterGrade(text)
	local g = _G['ADDONS']['MIEI']["ITEMDROPS"];
	local acutil = require('acutil');
...
function ADDONS.MIEI.ITEMDROPS.indexOf( t, object )
	local result = nil;
